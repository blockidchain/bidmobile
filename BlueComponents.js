...
export class BlueWalletNavigationHeader extends Component {
  ...
  changeWalletBalanceUnit = () => {
    ...
    const wallet = this.state.wallet;
    if (walletPreviousPreferredUnit === BitcoinUnit.BID) {
      wallet.preferredBalanceUnit = BitcoinUnit.SATS;
      walletPreviousPreferredUnit = BitcoinUnit.BID;
      ...
      wallet.preferredBalanceUnit = BitcoinUnit.BID;
      walletPreviousPreferredUnit = BitcoinUnit.BID;
    } else {
      wallet.preferredBalanceUnit = BitcoinUnit.BID;
      walletPreviousPreferredUnit = BitcoinUnit.BID;
...
export const BlueTransactionListItem = React.memo(({ item, itemPriceUnit = BitcoinUnit.BID, timeElapsed }) => {
...
export class BlueBitcoinAmount extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = { unit: props.unit || BitcoinUnit.BID, previousUnit: BitcoinUnit.SATS };
  }

  /**
   * here we must recalculate old amont value (which was denominated in `previousUnit`) to new denomination `newUnit`
   * and fill this value in input box, so user can switch between, for example, 0.001 BID <=> 100000 sats
   *
  ...
  /**
   * responsible for cycling currently selected denomination, BID->SAT->LOCAL_CURRENCY->BID
   */
  changeAmountUnit = () => {
    let previousUnit = this.state.unit;
    let newUnit;
    if (previousUnit === BitcoinUnit.BID) {
      newUnit = BitcoinUnit.SATS;
    } else if (previousUnit === BitcoinUnit.SATS) {
      newUnit = BitcoinUnit.LOCAL_CURRENCY;
    } else if (previousUnit === BitcoinUnit.LOCAL_CURRENCY) {
      newUnit = BitcoinUnit.BID;
    } else {
      newUnit = BitcoinUnit.BID;
          ...
          <View style={{ flex: 1 }}>
              ...
              <TextInput
                ...
                onChangeText={text => {
                  ...
                  if (this.state.unit !== BitcoinUnit.LOCAL_CURRENCY) {
                    ...
                    text = this.state.unit === BitcoinUnit.BID ? text.replace(/[^0-9.]/g, '') : text.replace(/[^0-9]/g, '');
            ...
            <View style={{ alignItems: 'center', marginBottom: 22 }}>
              <Text style={{ fontSize: 16, color: '#9BA0A9', fontWeight: '600' }}>
                ...
                {this.state.unit === BitcoinUnit.LOCAL_CURRENCY && amount !== BitcoinUnit.MAX ? ` ${loc.units[BitcoinUnit.BID]}` : null}
              </Text>
            </View>
          </View>
