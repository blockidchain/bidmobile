...
export default class SendDetails extends Component {
  ...
  processAddressData = data => {
    this.setState({ isLoading: true }, async () => {
      ...
      const dataWithoutSchema = data.replace('blockidcoin:', '').replace('BLOCKIDCOIN:', '');
      if (this.state.fromWallet.isAddressValid(dataWithoutSchema)) {
        ...
        let options;
        try {
          if (!data.toLowerCase().startsWith('blockidcoin:')) {
            data = `blockidcoin:${data}`;
          }
        ...
        console.log(options);
        if (btcAddressRx.test(address) || address.indexOf('blockidcoin1') === 0 || address.indexOf('BLOCKIDCOIN1') === 0) {
          const units = this.state.units;
          units[this.state.recipientsScrollIndex] = BitcoinUnit.BID; // also resetting current unit to BID
  ...
  async componentDidMount() {
    ...
    if (this.props.route.params.uri) {
      ...
      try {
        ...
        initialMemo = memo;
        this.setState({ addresses, memo: initialMemo, isLoading: false, amountUnit: BitcoinUnit.BID, payjoinUrl });
      } catch (error) {
        console.log(error);
      ...
      this.setState({ addresses, memo: initialMemo, isLoading: false, amountUnit: BitcoinUnit.BID });
    } else {
  ...
  importTransaction = async () => {
    ...
    try {
      const res = await DocumentPicker.pick({
        type:
          Platform.OS === 'ios'
            ? ['io.bidwallet.psbt', 'io.bidwallet.psbt.txn', DocumentPicker.types.plainText, 'public.json']
  ...
  render() {
    ...
    const allBalance = formatBalanceWithoutSuffix(balance, BitcoinUnit.BID, true);
