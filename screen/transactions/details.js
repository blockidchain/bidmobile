...
const TransactionsDetails = () => {
  ...
  const handleOnOpenTransactionOnBlockExporerTapped = () => {
    const url = `https://explorer.blockidcoin.network/tx/${tx.hash}`;
  ...
  return (
    <SafeBlueArea forceInset={{ horizontal: 'always' }} style={styles.root}>
      {isHandOffUseEnabled && (
        <Handoff title={`Bitcoin Transaction ${tx.hash}`} type="network.blockidcoin.wallet" url={`https://explorer.blockidcoin.network/tx/${tx.hash}`} />
      )}
