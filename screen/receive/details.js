...
const ReceiveDetails = () => {
  ...
  const [customUnit, setCustomUnit] = useState(BitcoinUnit.BID);
  ...
  /**
   * @returns {string} BID amount, accounting for current `customUnit` and `customUnit`
   */
...
/>
      {isHandOffUseEnabled && address !== undefined && showAddress && (
        <Handoff
          ...
          type="network.blockidcoin.wallet"
          url={`https://explorer.blockidcoin.network/address/${address}`}
        />
      )}
