...
describe('unit - DeepLinkSchemaMatch', function () {
  it('hasSchema', () => {
    ...
    assert.ok(DeeplinkSchemaMatch.hasSchema('bidwallet:bitcoin:12eQ9m4sgAwTSQoNXkRABKhCXCsjm2jdVG'));
    assert.ok(DeeplinkSchemaMatch.hasSchema('bidwallet:bitcoin:bc1qh6tf004ty7z7un2v5ntu4mkf630545gvhs45u7?amount=666&label=Yo'));
    assert.ok(DeeplinkSchemaMatch.hasSchema('bidwallet:bitcoin:BC1QH6TF004TY7Z7UN2V5NTU4MKF630545GVHS45U7?amount=666&label=Yo'));
    assert.ok(DeeplinkSchemaMatch.hasSchema('bidwallet:BITCOIN:BC1Q3RL0MKYK0ZRTXFMQN9WPCD3GNAZ00YV9YP0HXE'));
    assert.ok(DeeplinkSchemaMatch.hasSchema('bidwallet:BITCOIN:BC1Q3RL0MKYK0ZRTXFMQN9WPCD3GNAZ00YV9YP0HXE?amount=666&label=Yo'));
    assert.ok(
      DeeplinkSchemaMatch.hasSchema(
        'bidwallet:lightning:',
      ),
    );
  });
  ...
  it('isSafelloRedirect', () => {
    assert.ok(DeeplinkSchemaMatch.isSafelloRedirect({ url: 'bidwallet:?safello-state-token=TEST' }));
    assert.ok(!DeeplinkSchemaMatch.isSafelloRedirect({ url: 'bidwallet:' }));
  });
  ...
  it('navigationForRoute', async () => {
    const events = [
      ...
      {
        argument: { url: 'bidwallet:BITCOIN:BC1Q3RL0MKYK0ZRTXFMQN9WPCD3GNAZ00YV9YP0HXE?amount=666&label=Yo' },
        expected: [
          'SendDetailsRoot',
          { screen: 'SendDetails', params: { uri: 'BITCOIN:BC1Q3RL0MKYK0ZRTXFMQN9WPCD3GNAZ00YV9YP0HXE?amount=666&label=Yo' } },
        ],
      ...
      {
        argument: {
          url: 'bidwallet:?safello-state-token=TEST',
        },
        expected: [
          'BuyBitcoin',
          {
            safelloStateToken: 'TEST',
            uri: 'bidwallet:?safello-state-token=TEST',
            wallet: undefined,
          },
        ],
      },
      {
        argument: {
          url: 'bidwallet:setelectrumserver?server=electrum1.blockidcoin.network%3A443%3As',
        },
        expected: [
          'ElectrumSettings',
          {
            server: 'electrum1.blockidcoin.network:443:s',
          },
        ],
      },
      {
        argument: {
          url: 'bidwallet:setlndhuburl?url=https%3A%2F%2Flndhub.herokuapp.com',
        },
        expected: [
          'LightningSettings',
          {
  ...
  it('decodes bip21', () => {
    ...
    decoded = DeeplinkSchemaMatch.bip21decode(
      'bitcoin:bc1qnapskphjnwzw2w3dk4anpxntunc77v6qrua0f7?amount=0.0001&pj=https://btc.donate.kukks.org/BID/pj',
    );
    assert.strictEqual(decoded.options.pj, 'https://btc.donate.kukks.org/BID/pj');
  ...
  it('can decodeBitcoinUri', () => {
    assert.deepStrictEqual(
      ...
      ),
      {
        ...
        amount: 0.0001,
        memo: '',
        payjoinUrl: 'https://btc.donate.kukks.org/BID/pj',
      },
    );
  ...
  it('can work with some deeplink actions', () => {
    ...
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('bidwallet:setelectrumserver?server=electrum1.blockidcoin.network%3A443%3As'),
      'electrum1.blockidcoin.network:443:s',
    );
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('setelectrumserver?server=electrum1.blockidcoin.network%3A443%3As'),
      'electrum1.blockidcoin.network:443:s',
    );
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('ololo:setelectrumserver?server=electrum1.blockidcoin.network%3A443%3As'),
      false,
    );
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('setTrololo?server=electrum1.blockidcoin.network%3A443%3As'),
      false,
    );

    assert.strictEqual(
      DeeplinkSchemaMatch.getUrlFromSetLndhubUrlAction('bidwallet:setlndhuburl?url=https%3A%2F%2Flndhub.herokuapp.com'),
      'https://lndhub.herokuapp.com',
    );
    assert.strictEqual(
      DeeplinkSchemaMatch.getUrlFromSetLndhubUrlAction('bidwallet:setlndhuburl?url=https%3A%2F%2Flndhub.herokuapp.com%3A443'),
