...
class DeeplinkSchemaMatch {
  static hasSchema(schemaString) {
    ...
    return (
      lowercaseString.startsWith('blockidcoin:') ||
      lowercaseString.startsWith('lightning:') ||
      lowercaseString.startsWith('blue:') ||
      lowercaseString.startsWith('bidwallet:') ||
  ...
  static navigationRouteFor(event, completionHandler, context = { wallets: [], saveToDisk: () => {}, addWallet: () => {} }) {
    ...
    if (event.url.toLowerCase().startsWith('bidwallet:blockidcoin:') || event.url.toLowerCase().startsWith('bidwallet:lightning:')) {
      event.url = event.url.substring(11);
    } else if (event.url.toLocaleLowerCase().startsWith('bidwallet://widget?action=')) {
      event.url = event.url.substring('bidwallet://'.length);
    }
    ...
    if (isBothBitcoinAndLightning) {
      ...
      const urlObject = url.parse(event.url, true); // eslint-disable-line node/no-deprecated-api
      (async () => {
        if (urlObject.protocol === 'bidwallet:' || urlObject.protocol === 'lapp:' || urlObject.protocol === 'blue:') {
  ...
  /**
   * Extracts server from a deeplink like `bidwallet:setelectrumserver?server=electrum1.blockidcoin.network%3A443%3As`
  ...
  static getServerFromSetElectrumServerAction(url) {
    if (!url.startsWith('bidwallet:setelectrumserver') && !url.startsWith('setelectrumserver')) return false;
  ...
  /**
   * Extracts url from a deeplink like `bidwallet:setlndhuburl?url=https%3A%2F%2Flndhub.herokuapp.com`
   * returns FALSE if none found
   *
   * @param url {string}
   * @return {string|boolean}
  ...
  static getUrlFromSetLndhubUrlAction(url) {
    if (!url.startsWith('bidwallet:setlndhuburl') && !url.startsWith('setlndhuburl')) return false;
  ...
  static isBitcoinAddress(address) {
    address = address.replace('blockidcoin:', '').replace('VELES:', '').replace('blockidcoin=', '').split('?')[0];
  ...
  static isBothBitcoinAndLightning(url) {
    if (url.includes('lightning') && (url.includes('blockidcoin') || url.includes('VELES'))) {
      const txInfo = url.split(/(blockidcoin:|VELES:|lightning:|lightning=|blockidcoin=)+/);
      let bitcoin;
      let lndInvoice;
      for (const [index, value] of txInfo.entries()) {
        try {
          // Inside try-catch. We dont wan't to  crash in case of an out-of-bounds error.
          if (value.startsWith('blockidcoin') || value.startsWith('VELES')) {
            bitcoin = `blockidcoin:${txInfo[index + 1]}`;
  ...
  static bip21decode(uri) {
    return bip21.decode(uri.replace('VELES:', 'blockidcoin:'));
  }
